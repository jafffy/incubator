...
static tlcdtext context;

void tlcd_init() //초기화
{
	context.fd = open(TLCD_RIVER_PATH, 0_RDWR);
};
...
int IsBusy(void) // 이전의 명령이 수행중인지 확인
{
...
int writeCmd(unsigned short cmd) //TLCD에 명령을 내림
{
...
int setDDRAMAddr(int x , int y) //커서와 index을 설정
//	printf("x :%d , y:%d \n",x,y);
	if(IsBusy())
	{
...
int writeCh(unsigned short ch) // 현재 address index에 한 문자를 씀
{
...
int setCursorMode(int bMove , int bRightDir)// 현제 커서를 앞 뒤로 움직임
{
...
int writeStr(char* str) //현재 address index에 문자 쓰기
{
	unsigned char wdata;
	int i;
...
int clearScreen(int nline) // 라인별 지우기
{
	int i;
	if (nline == 0)
	{
