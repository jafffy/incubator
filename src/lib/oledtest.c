...
//putty에서 sample.img파일이 어디에 있는지 확인하기
void doHelp(void)
{
	printf("Usage:\n");
...
oledContext oledCont;

void oled_open()//드라이버 오픈
{
	oledCont.fd = open(OLED_DRIVER_PATH,O_RDWR);
}
...
int reset()//oled chip reset
{
	unsigned short wdata ;

	wdata = RST_BIT_MASK;// 1110 1111 1111 1111 
	write(oledCont.fd,&wdata , 2 );//wdata의 값을 fd에 2byte를 쓴다.
	usleep(2000);
...
int writeCmd(int size , unsigned short* cmdArr)//명령 배열?을 받아서 fd에 2byte씩 쓴다.
{
	int i ;
	unsigned short wdata;
...
int writeData(int size , unsigned char* dataArr)
{
	...
	wdata = CS_BIT_MASK & DC_BIT_MASK & WD_BIT_MASK & (CMD_WRITE_RAM | 0xFF00) ; // 1111 0001 0101 1100
	write(oledCont.fd,&wdata,2);
...
int setting_init_oled(void)//초기 디바이스 설정
{
...
//	unsigned char wdata[10];
	...
	*/
	...
	//	break;
	//case MODE_IMAGE:
	imageLoading("./a.png");//이미지가 있는 곳의 메모리 주소 
	//	break;
	//case MODE_INIT:
//		setting_init_oled();
//		break;
