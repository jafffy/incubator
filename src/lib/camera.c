...
int CreateCamera(int index)
{
	...
	m_cam_fd = open(CAMERA_DEV_NAME, O_RDWR);//CAM_Driver
	if (m_cam_fd < 0) {
               ...
               unsigned char* r, unsigned char* g, unsigned char* b)
{
	...
	R=amp*(0.004565*y+0.000001*u+0.006250*v-0.872);//yuv로 rgv만든다
...
static void Draw(unsigned char *displayFrame, unsigned char *videoFrame,int videoWidth, int videoHeight, int dFrameWidth,int dFrameHeight)
{ 
	int    x,y;
	...
	for ( y = 0 ; y < videoHeight ; y++ )
	{
		for(x = 0; x < videoWidth ; x++ )
		{
			...
			yuv_a_rgb(Y, U, V, &R, &G, &B);//generate the value of R,G,B
...
static void NewDrawFromRGB565(unsigned char *displayFrame, unsigned char *videoFrame,int s_x, int s_y, int videoWidth, int videoHeight, \
				 int dFrameWidth,int dFrameHeight)//     videoWidth*videoHeight=size of video, dFrameWidth*dFrameHeight=size of LCD
{ 
	int    x,y;
	int lineLeng ;
	unsigned short *videptrTemp;
	...
	if( s_x>=640|s_y>=320 ){//if starting point of s_x is over 640 or starting point of s_y is over 320 then exit function.
...
#define  FBDEV_FILE "/dev/fb0"//프레임 버퍼 드라이버
...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		NewDrawFromRGB565(fb_mapped, m_buffers_preview[index].virt.p, 100, 100, CAMERA_PREVIEW_WIDTH, CAMERA_PREVIEW_HEIGHT, screen_width, screen_height);
